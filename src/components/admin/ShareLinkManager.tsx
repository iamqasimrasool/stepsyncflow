...
export default function ShareLinkManager({ sopId }: { sopId: string }) {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/sops/${sopId}/share`);
        if (res.status === 404) {
          setShareLink(null);
          return;
        }
  ...
  const handleCreate = async () => {
    setSaving(true);
    try {
      const res = await fetch(`/api/sops/${sopId}/share`, { method: "POST" });
      if (!res.ok) throw new Error("Failed");
      const data = await res.json();
      setShareLink(data.shareLink);
  ...
  const handleRotate = async () => {
    setSaving(true);
    try {
      const res = await fetch(`/api/sops/${sopId}/share/rotate`, {
        method: "POST",
      });
      if (!res.ok) throw new Error("Failed");
      const data = await res.json();
  ...
  return (
    <div className="rounded-2xl border border-white/60 bg-white/80 p-4 space-y-4">
      <div>
        <h3 className="text-lg font-semibold">Public link</h3>
        <p className="text-sm text-muted-foreground">
          Share this flow publicly with optional password protection.
        </p>
      </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-col gap-2 md:flex-row md:items-center">
              ...
              <div className="flex gap-2">
                ...
                <Button type="button" variant="outline" onClick={handleRotate} disabled={saving}>
                  Rotate
                </Button>
              </div>
            </div>
